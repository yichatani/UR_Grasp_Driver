...
class DataCollector:
    ...
    def record_step(self):
        ...
        try:
            ...
            current_gripper_width = self.coordinator.gripper_width
        except Exception as e:
        ...
        if self.previous_state is not None:
            ...
            self.buffer['current_gripper_width'].append(self.previous_state['gripper_width'])
            ...
            self.buffer['next_gripper_width'].append(current_gripper_width)
            # Observations
    ...
    def set_folder(self):
        ...
        self.end_episode_n = self._get_latest_episode_number() if self.auto_episode else 0
        episode_dir = os.path.join(self.output_dir, f'episode_{self.end_episode_n:04d}')
        os.makedirs(episode_dir, exist_ok=True)
        return episode_dir


    def end_episode(self, episode_dir):
        ...
        np.savez_compressed(
            ...
            current_gripper_width = np.array(self.buffer['current_gripper_width']),
            # action
            ...
            target_gripper_width=np.array(self.buffer['next_gripper_width']),
        )
    ...
    def execute_grasp(self, color_image, depth_image):
        ...
        try:
            ...
            if self.executionType == "Execute":
                ...
                try:
                    ### Begin record data
                    episode_dir = self.record.set_folder()
                    subfolder_dir = os.path.join(episode_dir, '00')
                    os.makedirs(subfolder_dir, exist_ok=True)
                    ###
                    self.gripper_width = selected_grasp.width
                    self.gripper_controller.adjust_gripper_width(selected_grasp.width)
                    self.move_to_grasp()  
                    self.pick_object()
                    self.gripper_width = 0.8
                    self.record.end_episode(subfolder_dir)


                    subfolder_dir2 = os.path.join(episode_dir, '01')
                    os.makedirs(subfolder_dir2, exist_ok=True)

                    self.move_to_release_position()
                    self.release_object()
                    self.gripper_width = 0.0
                    self.record.end_episode(subfolder_dir2)


                    subfolder_dir3 = os.path.join(episode_dir, '02')
                    os.makedirs(subfolder_dir3, exist_ok=True)
                    ...
                    self.record.end_episode(subfolder_dir3)
                    self.record.stop()
                    record_thread.join()
                    self.record = DataCollector(self, output_dir="./collected_data", auto_episode=True)

                finally:
                    # Ensure the record thread finishes before proceeding
                    record_thread.join()
