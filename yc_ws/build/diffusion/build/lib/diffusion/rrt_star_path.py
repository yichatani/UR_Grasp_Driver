...
class RRTStarPlanner:
    ...
    def plan(self, 
        ...
        for _ in range(max_iterations):
            ...
            new_cost = self.costs[nearest_idx] + np.linalg.norm(nearest_node - new_state)
            ...
            if np.linalg.norm(new_state - goal) < self.max_step:
                if self.is_valid_edge(new_state, goal):
                    self.add_node(goal, len(self.nodes) - 1, new_cost + np.linalg.norm(new_state - goal))
...
if __name__ == "__main__":
    ...
    goal = np.array([5, 5, 5])
    bounds = (np.array([-1, -1, -1]), np.array([6, 6, 6]))
